  ...
  def destroy
    respond_to do |format|
      if @equipment.loans.any?
        format.html do
          redirect_to equipment_index_path, alert: 'Não é possível apagar um equipamento que já foi emprestado.'
        end
        format.json do
          render json: { error: 'Equipamento com empréstimos não pode ser excluído.' }, status: :unprocessable_entity
        end

      elsif @equipment.status != 'disponivel'
        format.html do
          redirect_to equipment_path(@equipment), alert: 'Só é possível excluir equipamentos disponíveis.'
        end
        format.json do
          render json: { error: 'Equipamento não disponível para exclusão.' }, status: :unprocessable_entity
        end

      else
        @equipment.destroy!
        format.html do
          redirect_to equipment_index_path, status: :see_other, notice: 'Equipamento apagado com sucesso.'
        end
